...
void Controller::chooseCharacterRace() {
    cout << endl << "Welcome to Chamber Crawler!" << endl;
    cout << endl << "Choose your character race:" << endl;
    cout << "s: Shade - Gets 50% increase to final score" << endl;
    cout << "d: Drow - Potion effects get magnified by 1.5" << endl;
    cout << "v: Vampire - Gains 5 HP for every successful attack (no max HP)" << endl;
    cout << "g: Goblin - Recovers 5 HP every turn" << endl;
    cout << "t: Troll - Steals 5 gold from every slain enemy" << endl;
...
void Controller::playGame() {
    ...
    while (!gameOver && cin >> command) {
        ...
        if (command == "q") {
            ...
            cout << "Type u ID"
            cout << "Items: " << endl;
            int counter = 0; // counter to keep track that we don't print too many items on one line
            for (unsigned int i = 0; i < inventory.size(); i++) {
                if (counter == 5) cout << "        ";
                cout << inventory[i]->getItemType() << "  ";
                counter += 1;
            }
        }
        else if (command == "ea") {
        ...
        if (endTurn) return;
        if (command == "a" || command == "u" || command == "p") {
            // Attacking
            string direction;
            cin >> direction;
                ...
                if (command == "a") {
                    ...
                    // Put the item in the player's inventory
                        ...
                        // Do nothing
                            ...
                            if (items[i]->getItemLoc()== objectLoc) {
                                view->setTile(items[i]->getItemLoc(), '.'); // set item location to '.' because it was picked up
                                inventory.push_back(items[i]->copy());
                                delete items[i];
